...
const char* CARDSET_IMAGE = "C:/Users/123/Desktop/CardSet.bmp";
const char* CARDBACK_1 = "C:/Users/123/Desktop/card_back_1.bmp";
const char* CARDBACK_2 = "C:/Users/123/Desktop/card_back_2.bmp";
...
Uint8 player_figth_card(int x, int y, int width, int height, player_t* player, field_t* field) {
	...
	if (card_num == UINT8_MAX)
		return 0;
	if (card != UINT8_MAX 
		&& !(get_suit(check) != field->trump && get_suit(card) == field->trump)
		&& (get_card_price(check) > get_card_price(card))
		|| (get_suit(check) == field->trump && get_suit(card) != field->trump)) {
		field->cards[card_num + 6] = check;
		player_card_pop(player, player->checked);
		field->used[check % 13] = 1;
		return 1;
	}
...
Uint8 enemy_fight(player_t* player, player_t** enemies, field_t* field, Uint8 move, Uint8 mode) {
	...
	if (fighter == UINT8_MAX)
		return UINT8_MAX;
	for (i = 0; i < 6; i++) {
		if (field->cards[i] == UINT8_MAX || field->cards[i + 6] != UINT8_MAX)
			continue;
...
int side(Uint8 mode) {
	...
	SDL_Event e;
	Uint8 movement = 0, move = 0, step = 0, finish_move = 0, player_ready = 0
	...
	while (!quit){
		...
		else if (e.type == SDL_MOUSEBUTTONDOWN && (!move || mode == move) && me.checked == UINT8_MAX) {
			movement= 1;
		...
		if (player_ready && finish_move) {
			...
			pl = get_ingame_player(&me, &enemies, move, mode);
			if (pl == 3 && !player_take)
				move = 0;
				...
				move = ((get_ingame_player(&me, &enemies, pl + 1, mode))) % 3;
			player_take = enemy_take = 0;
		}
		if (step == FIGHT_STEP && move != mode) {
			enemy_take = enemy_fight(&me, &enemies, &field, move, mode);
			if (enemy_take)
				finish_move = 1;
			step = FORWARD_STEP;
		}
		if (step == FORWARD_STEP && move) {
			Uint8 pl = get_ingame_player(&me, &enemies, move, mode);
